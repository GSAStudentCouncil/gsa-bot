...
cronjob.add("* * * * *", function () {
    var channel = manager.getChannelById(393252027657624); // 봇 테스트 방
 ...
 * 오픈채팅에 들어왔을 때 반응
 */
app.on("join", function (chat, channel) {
    channel.send(chat.joinUsers[0].nickName + "님 안녕하세요");
});
/**
 * 단체톡방에 초대했을 때 반응
 */
app.on("invite", function (chat, channel) {
    channel.send(chat.inviteUser.nickName + "님이" + chat.invitedUsers.map(function (e) {
        return e.nickName;
    }).join(",") + "님을 초대했습니다");
});
...
app.on("leave", function (chat, channel) {
    if (chat.isKicked()) {
 ...
 * 권한이 바뀔 때
 */
app.on("member_type_change", function (chat, channel) {
    if (chat.isDemote()) {
 ...
 * 오픈채팅방에서 프로필을 바꿀 때
 */
app.on("open_profile_change", function (beforeUser, afterUser, channel) {
