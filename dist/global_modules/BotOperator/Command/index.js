...
try {
  class Command {
    ...
    createManual(contents) {
      let ret = [`🧩 '${this.name}' 명령어 도움말${compress}`, '——————————', this.description, '', ...contents, ''];
      if (this.cronJobs.length > 0) {
        ret.push('📌 자동 실행 주기');
        ret.push('——');
        ret.push(...this.cronJobs.map(({
          comment
        }) => `· ${comment}`));
        ret.push('');
      }
  ...
  class IntArg extends Arg {
    ...
    toRegExp() {
      ...
      if (!this.many) return ret;else return new RegExp(`(?:${ret.source}\\s?)${this.includeEmpty ? "*" : "+"}`);
    }
    parse(value) {
      ...
      if (this.many) {
        ...
        if (this.min && ret.some(v => v < this.min)) return false;else if (this.max && ret.some(v => v > this.max)) return false;else return ret;
      } else {
        if (value == null) return null;
        let ret = Number(value);
        if (this.min && ret < this.min) return false;else if (this.max && ret > this.max) return false;else return ret;
      }
    }
  }
  class StrArg extends Arg {
    ...
    toRegExp() {
      ...
      let ret;
      if (this.length) ret = new RegExp(`\\S{${this.length}}`);else if (this.minLength && this.maxLength) ret = new RegExp(`\\S{${this.minLength},${this.maxLength}}`);else if (this.minLength) ret = new RegExp(`\\S{${this.minLength},}`);else ret = new RegExp(`\\S${this.includeEmpty ? "*" : "+"}`);
      if (!this.many) return ret;else return new RegExp(`(?:${ret.source}\\s?)${this.includeEmpty ? "*" : "+"}`);
    }
    parse(value) {
      if (value != null && !this.toRegExp().test(value)) return false;
  ...
  class StructuredCommand extends Command {
    constructor(options) {
      ...
      let regexApplied = this.usage.replace(/\s*<.+?>/g, m => {
        ...
        if (k == null) throw new TypeError(`Invalid type: ${type}`);
        ...
        if (args[args.length - 1].includeEmpty) return `(?:${ret})?`;else return ret;
      });
      this.args = args;
    ...
    manual(formats) {
      ...
      let usage = this.usage.replace(/<.+?>/g, m => {
        ...
        if (this.args.find(arg => arg.name === name).many) name = `${name}...`;
        return `<${name}>`;
      });
      ret.push(usage);
      let args = this.args.map((arg, i) => {
        ...
        Object.keys(arg).forEach(key => {
          if (arg[key] && `_${key}_str` in arg)
            // null, undefined, 0, false 등이 아닐 경우
            properties.push(`${arg[key]}${arg[`_${key}_str`]}`);else {
  ...
  class NaturalCommand extends Command {
    constructor(options) {
      ...
      let dictionary = IS_DIST ? JSON.parse(FileStream.read(`${S}/${this.dictionaryPath}`)) : require(`./${this.dictionaryPath}`);
      this.map = {};
    ...
    manual(formats) {
      ...
      for (let key in this.query) {
        if (key === 'datetime' || key === 'duration') continue;
        ...
        if (typeof this.query[key] === 'function') tmp += ` (생략 시 기본값 = ${humanize(this.query[key]())})`;else if (this.query[key] != null && !Number.isNaN(this.query[key])) tmp += ` (생략 시 기본값 = ${humanize(this.query[key])})`;
        ret.push(tmp);
      }
      if (this.useDateParse) {
        let tmp;
        if (this.useDuration) tmp = `· 기간을 의미하는 용어 (ex. 다음 주까지, 내일부터 모레 저녁까지, ...)`;else tmp = `· 날짜 및 시간을 의미하는 용어 (ex. 3월 14일, 내일 저녁, 4일 뒤 5시, ...)`;
        let humanize = value => value instanceof DateTime ? value.humanize() : value;
        if (typeof this.query['datetime'] === 'function') tmp += ` (생략 시 기본값 = ${humanize(this.query['datetime']())})`;else if (this.query['datetime'] != null && !Number.isNaN(this.query['datetime'])) tmp += ` (생략 시 기본값 = ${humanize(this.query['datetime'])})`;
        ret.push(tmp);
      }
      let manual = super.createManual(ret);
      for (let fmt in formats) {
  ...
  const CronLog = (cmd, idx, datetime) => {
    return [`호출된 명령어: Cronjob of ${cmd instanceof StructuredCommand ? "StructuredCommand" : "NaturalCommand"}(${cmd.icon} ${cmd.name})`, `명령어 인자: ${JSON.stringify({
      idx,
      datetime
    })}`, `시간: ${datetime.toString()}`].join('\n');
  };
  class Registry {
    ...
    register(command, logRoom) {
      ...
      for (let cmd of this.data) {
        if (cmd.name === command.name) throw new Error(`Command with name "${command.name}" already exists`);
      }
      ...
      for (let idx = 0; idx < command.cronJobs.length; idx++) {
        ...
        this.cronManager.add(cron, () => {
          ...
          if (isValidChannel(logRoom)) logRoom.send(CronLog(command, idx, datetime)).then(() => {}, e => Log.e(e.stack));
        }, cronOpt);
      }
    }
    ...
    get(chat, channel, debugRooms, isDebugMod) {
      ...
      for (let cmd of this.data) {
        ...
        if (isDebugMod && !channelToIdArray(debugRooms).includes(channel.id)) continue;

        // 디버그 모드가 아닐 때, 방이 포함되어 있지 않을 경우 실행하지 않음
        else if (cmd.channels.length !== 0 && !channelToIdArray([...cmd.channels, ...debugRooms]).includes(channel.id)) continue;
        ...
        if (cmd instanceof StructuredCommand) {
          ...
          if (cmd.useDateParse) {
            if (cmd.useDuration) {
              ...
              let {
                ...
                string
              } = DateTime.parseDuration(filteredText, true, cmd.filterIncludeEnding);
              ...
              let {
                parse,
                string
              } = DateTime.parse(filteredText, true, cmd.filterIncludeEnding);
              if (parse != null) {
                args.datetime = parse;
                filteredText = string;
              }
          ...
          if (chat.filteredText.replace(/\s+/g, '').length > cmd.margin) continue;
          let is_full = true;
          for (let key in args) {
            if (args[key] == null) {
              is_full = false;
