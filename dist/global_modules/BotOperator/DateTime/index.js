...
const getCultureInfo = locale => {
  let ret = IS_DIST ? JSON.parse(FileStream.read(`${android.os.Environment.getExternalStorageDirectory().getAbsolutePath()}/msgbot/global_modules/BotOperator/DateTime/globalization/${locale}.json`)) : require(`./globalization/${locale}.json`);
  if (ret == null) throw Error(`Invalid Locale ${locale}. (${android.os.Environment.getExternalStorageDirectory().getAbsolutePath()}/msgbot/global_modules/BotOperator/DateTime/globalization/${locale}.json is not exist)`);
  return ret;
};
class TimeDelta {
...
class DateTime {
  constructor(datetimeObject, locale) {
    ...
    if (datetimeObject instanceof $D) {
      ...
      let dt;
      if (datetimeObject instanceof DateTime) dt = datetimeObject;else if (typeof datetimeObject === 'number') dt = DateTime.fromNumber(datetimeObject);else if (typeof datetimeObject === 'object' && !Array.isArray(datetimeObject)) dt = DateTime.fromObject(datetimeObject);else if (typeof datetimeObject === 'string') dt = DateTime.fromString(datetimeObject, this._locale);else throw new TypeError('`datetimeObject` must be $D, datetime, number, object, or string');
  ...
  humanize(ignoreTime = false) {
    ...
    if (!isSpecial) {
      ...
      if (lastWeekMonDiff <= dayDiff && dayDiff <= nextWeekSunDiff) {
        // 저번 주 ~ 다음 주
        if (dayDiff - lastWeekMonDiff < 7) str.date = `지난 주 ${this.weekdayName}`;else if (nextWeekSunDiff - dayDiff < 7) str.date = `다음 주 ${this.weekdayName}`;else if (dayDiff < 0) str.date = `이번 주 ${this.weekdayName}`; // 이번 주의 지나간 요일은 '이번 주' 포함
      ...
      // }
      else if (this.year === now.year) str.date = `${this.month}월 ${this.day}일`;else str.date = `${this.year}년 ${this.month}월 ${this.day}일`;
    }
    ...
    if (!ignoreTime) {
      if (this.hour === 0 && this.minute === 0 && this.second === 0) str.time = ""; // '오늘 자정'은 마치 내일 0시를 말하는 것 같아서 그냥 '오늘'로 표시
        ...
        if (amountSec < 6 * 60 * 60) {
          // 6시간 이내
          isRelative = true;
          if (hour !== 0) str.time += `${hour}시간 `;
          if (minute !== 0) str.time += `${minute}분 `;
          if (second !== 0) str.time += `${second}초 `;
          // 밀리초는 수행 시간에도 영향을 받고, 너무 세부적이므로 무시. 나중에 config 로 설정할 수 있게?

          if (str.time !== '') str.time = str.time.trim() + ` ${sign}`;
    ...
    if (!ignoreTime && this.eq(now, true)) return '지금';else if (!ignoreTime && isRelative)
      ...
      // 오늘인데 시간이 있으면 날짜를 생략
      return str.time;else return `${str.date} ${str.time}`.trim();
  }
  toNumber() {
    ...
    const relative_parse = () => {
      ...
      if (arr2 != null) {
        ...
        while ((arr = RE_RELATIVE.exec(dateString)) != null) {
          ...
          } else ret[key] = (ret[key] ?? 0) + set(direction, diff);
      ...
      while ((arr = RE_RELATIVE2.exec(dateString)) != null) {
        ...
        } else ret[unitMap[unit]] = (ret[unitMap[unit]] ?? 0) + diff_num;
      }
    ...
    if (iso_parsed != null) {
      ...
      if (Object.keys(common_parsed).length === 0 && Object.keys(relative_parsed).length === 0) ret = {
        ...
        for (let i = 0; i < units.length; i++) {
          if (!relative_parsed.defaultToNow && i > lastIndex) break;
        ...
        for (let unit of units) {
          ...
          else if (common_parsed[unit] != null || relative_parsed[unit] != null) parsed[unit] = common_parsed[unit] ?? relative_parsed[unit];
        }
        filteredString = filteredString.replace(/\s+/g, ' ');
        ret = {
          parse: parsed,
  ...
  static parseDuration(dateString, getString = false, filterIncludeEnding = true, std = DateTime.now(), locale = 'ko-KR') {
    ...
    if (split.length === 1) {
      ...
      let {
        parse: leftParse,
        string: leftFString
      } = DateTime.parse(left, true, filterIncludeEnding, false, std, locale);
      let {
        parse: rightParse,
        string: rightFString
      } = DateTime.parse(right, true, filterIncludeEnding, false, std, locale);
  ...
  static getWeekdayFromName(weekDayName, startOnMon = false, locale = 'ko-KR') {
    ...
    if (ret === -1) ret = WW.map(e => e.toLowerCase()).indexOf(weekDayName.toLowerCase());
