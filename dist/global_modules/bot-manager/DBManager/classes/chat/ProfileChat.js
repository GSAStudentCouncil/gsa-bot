'use strict';

function _typeof(o) {
  "@babel/helpers - typeof";

  return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) {
    return typeof o;
  } : function (o) {
    return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o;
  }, _typeof(o);
}
function D() {
  var g = ['lue\x20', '252940wmiEcR', 'nfJJM', 'operty', 'construc', 'profile', 'call', '__extend', '11GtXrIy', '301648MzGxbq', 'wIrRW', 'MtqHC', 'ProfileC', 'HBLTz', 'hasOwnPr', '__esModu', 'typeOf', '\x20is\x20not\x20', 'DssaI', 'hat', '242938EDyxJF', 'Class\x20ex', 'Chat', 'raw', 'prototyp', 'attachme', 'a\x20constr', 'tor', '56UkMqzM', 'definePr', '657RwMOmh', 'qzsaE', 'isProfil', '2390652GuIBhO', 'setProto', 'Yidqg', '3157365CPfmkw', 'uzHlB', '3409608xprexz', '1061608grIPhJ', './Chat', '__proto_', 'uctor\x20or'];
  D = function D() {
    return g;
  };
  return D();
}
var R = w,
  h = w;
(function (z, B) {
  var n = {
      z: 0x217,
      B: 0x20c,
      O: 0x1fc,
      I: 0x1fe,
      r: 0x1f4,
      d: 0x1ff
    },
    U = w,
    X = w,
    O = z();
  while (!![]) {
    try {
      var I = parseInt(U(0x20b)) / 0x1 * (parseInt(X(n.z)) / 0x2) + -parseInt(U(0x1f9)) / 0x3 + -parseInt(X(n.B)) / 0x4 + parseInt(U(n.O)) / 0x5 + parseInt(X(n.I)) / 0x6 + -parseInt(U(n.r)) / 0x7 * (-parseInt(U(n.d)) / 0x8) + -parseInt(U(0x1f6)) / 0x9 * (parseInt(U(0x204)) / 0xa);
      if (I === B) break;else O['push'](O['shift']());
    } catch (r) {
      O['push'](O['shift']());
    }
  }
})(D, 0xd689e);
function w(z, B) {
  var O = D();
  return w = function w(I, r) {
    I = I - 0x1f1;
    var d = O[I];
    return d;
  }, w(z, B);
}
var __extends = void 0 && (void 0)[R(0x20a) + 's'] || function () {
  var W = {
      z: 0x218,
      B: 0x203,
      O: 0x1f2
    },
    p = {
      z: 0x210,
      B: 0x20e,
      O: 0x205,
      I: 0x1f7,
      r: 0x20d,
      d: 0x1fb,
      t: 0x21b,
      c: 0x21b
    },
    N = {
      z: 0x1fa,
      B: 0x1fd
    },
    J = R,
    q = R,
    z = {
      'uzHlB': function uzHlB(O, I, r) {
        return O(I, r);
      },
      'NqXXB': function NqXXB(O, I) {
        return O != I;
      },
      'HBLTz': 'function',
      'MtqHC': function MtqHC(O, I) {
        return O !== I;
      },
      'nfJJM': J(W.z) + 'tends\x20va' + J(W.B),
      'qzsaE': function qzsaE(O, I) {
        return O(I);
      },
      'wIrRW': J(0x214) + q(W.O) + q(0x202) + '\x20null',
      'Yidqg': function Yidqg(O, I, r) {
        return O(I, r);
      }
    },
    _B = function B(O, I) {
      var s = {
          z: 0x21b,
          B: 0x211,
          O: 0x206
        },
        K = q,
        H = q;
      return _B = Object[K(N.z) + H(0x213)] || {
        '__proto__': []
      } instanceof Array && function (d, c) {
        var Z = K;
        d[Z(0x201) + '_'] = c;
      } || function (d, c) {
        var u = H,
          G = H;
        for (var m in c) Object[u(s.z) + 'e'][G(s.B) + G(s.O)][u(0x209)](c, m) && (d[m] = c[m]);
      }, z[K(N.B)](_B, O, I);
    };
  return function (O, I) {
    var V = {
        z: 0x1f3
      },
      j = q,
      o = q;
    if (z['NqXXB'](z[j(p.z)], _typeof(I)) && z[o(p.B)](null, I)) throw new TypeError(z[o(p.O)] + z[o(p.I)](String, I) + z[o(p.r)]);
    function d() {
      var F = o,
        b = j;
      this[F(0x207) + F(V.z)] = O;
    }
    z[o(p.d)](_B, O, I), O['prototyp' + 'e'] = null === I ? Object['create'](I) : (d[j(p.t) + 'e'] = I[o(p.c) + 'e'], new d());
  };
}();
Object[R(0x1f5) + h(0x206)](exports, h(0x212) + 'le', {
  'value': !0x0
}), exports[R(0x20f) + R(0x216)] = void 0x0;
var Chat_1 = require(R(0x200)),
  ProfileChat = function (z) {
    var v = {
        z: 0x21b,
        B: 0x1f8
      },
      k = {
        z: 0x21a,
        B: 0x1f1
      },
      C = R,
      E = h,
      B = {
        'DssaI': function DssaI(I, r, d) {
          return I(r, d);
        },
        'FcOUO': C(0x208)
      };
    function O(I) {
      return z['call'](this, I) || this;
    }
    return B[E(0x215)](__extends, O, z), Object[E(0x1f5) + 'operty'](O['prototyp' + 'e'], B['FcOUO'], {
      'get': function get() {
        var T = E,
          P = C;
        return this[T(k.z)][T(k.B) + 'nt'];
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), O[C(v.z) + 'e'][C(v.B) + 'e'] = function () {
      return !0x0;
    }, O;
  }(Chat_1[h(0x219)]);
exports[R(0x20f) + 'hat'] = ProfileChat;