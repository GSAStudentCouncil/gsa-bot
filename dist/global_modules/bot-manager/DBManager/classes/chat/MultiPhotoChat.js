'use strict';

function _typeof(o) {
  "@babel/helpers - typeof";

  return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) {
    return typeof o;
  } : function (o) {
    return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o;
  }, _typeof(o);
}
var X = w,
  F = w;
function D() {
  var v = ['definePr', 'attachme', 'photoLis', 'construc', 'hoto', './Chat', 'THHhe', '172740edyAeW', 'Class\x20ex', '42AnLWCc', 'setProto', '615149vtsLxV', 'qyGPO', '20337603kdsxMG', '7zvwUla', '\x20is\x20not\x20', 'prototyp', '293775pRiWTf', 'isMultiP', 'Sepzl', '_raw', 'IFZRQ', 'tor', '40EJFrVM', 'MultiPho', '5340168ONqFnf', 'RxzVj', 'call', 'tends\x20va', '__extend', 'JyCTg', '1244960YLbaIb', 'VgoJl', 'hasOwnPr', 'create', 'operty', '4706360ysBaBr', 'toChat', 'udZyH', '\x20null', '__esModu', '2CzPknq'];
  D = function D() {
    return v;
  };
  return D();
}
(function (z, B) {
  var P = {
      z: 0x1f2,
      B: 0x20a,
      O: 0x204,
      I: 0x1fc,
      r: 0x217,
      d: 0x20c,
      t: 0x200
    },
    m = w,
    U = w,
    O = z();
  while (!![]) {
    try {
      var I = parseInt(m(0x1fe)) / 0x1 * (-parseInt(U(P.z)) / 0x2) + -parseInt(m(0x1fa)) / 0x3 * (-parseInt(m(P.B)) / 0x4) + parseInt(U(P.O)) / 0x5 * (-parseInt(U(P.I)) / 0x6) + -parseInt(U(0x201)) / 0x7 * (parseInt(m(P.r)) / 0x8) + -parseInt(U(P.d)) / 0x9 + parseInt(m(0x212)) / 0xa + parseInt(m(P.t)) / 0xb;
      if (I === B) break;else O['push'](O['shift']());
    } catch (r) {
      O['push'](O['shift']());
    }
  }
})(D, 0x53460);
var __extends = void 0 && (void 0)[X(0x210) + 's'] || function () {
  var W = {
      z: 0x1fb,
      B: 0x20f,
      O: 0x202
    },
    p = {
      z: 0x206,
      B: 0x211,
      O: 0x1f9,
      I: 0x20d,
      r: 0x1ef,
      d: 0x1ff,
      t: 0x213,
      c: 0x203,
      W: 0x215,
      l: 0x203
    },
    V = {
      z: 0x209
    },
    N = {
      z: 0x1fd,
      B: 0x208,
      O: 0x213
    },
    s = {
      z: 0x203,
      B: 0x214,
      O: 0x216
    },
    R = X,
    J = X,
    z = {
      'IFZRQ': function IFZRQ(O, I) {
        return O instanceof I;
      },
      'VgoJl': function VgoJl(O, I, r) {
        return O(I, r);
      },
      'JEBCP': function JEBCP(O, I) {
        return O != I;
      },
      'Sepzl': 'function',
      'JyCTg': function JyCTg(O, I) {
        return O !== I;
      },
      'THHhe': function THHhe(O, I) {
        return O + I;
      },
      'RxzVj': R(W.z) + R(W.B) + 'lue\x20',
      'udZyH': function udZyH(O, I) {
        return O(I);
      },
      'qyGPO': J(W.O) + 'a\x20constr' + 'uctor\x20or' + J(0x1f0)
    },
    _B = function B(O, I) {
      var q = J,
        K = J;
      return _B = Object[q(N.z) + 'typeOf'] || z[q(N.B)]({
        '__proto__': []
      }, Array) && function (r, d) {
        r['__proto_' + '_'] = d;
      } || function (r, d) {
        var H = K,
          Z = q;
        for (var c in d) Object[H(s.z) + 'e'][H(s.B) + Z(s.O)]['call'](d, c) && (r[c] = d[c]);
      }, z[q(N.O)](_B, O, I);
    };
  return function (O, I) {
    var u = J,
      G = J;
    if (z['JEBCP'](z[u(p.z)], _typeof(I)) && z[u(p.B)](null, I)) throw new TypeError(z[u(p.O)](z[u(p.I)], z[G(p.r)](String, I)) + z[G(p.d)]);
    function d() {
      var j = u,
        o = u;
      this[j(0x1f6) + j(V.z)] = O;
    }
    z[u(p.t)](_B, O, I), O[G(p.c) + 'e'] = null === I ? Object[u(p.W)](I) : (d['prototyp' + 'e'] = I[u(p.l) + 'e'], new d());
  };
}();
Object['definePr' + F(0x216)](exports, F(0x1f1) + 'le', {
  'value': !0x0
}), exports[F(0x20b) + X(0x1ee)] = void 0x0;
var Chat_1 = require(X(0x1f8)),
  MultiPhotoChat = function (z) {
    var L = {
        z: 0x1f5,
        B: 0x1f3,
        O: 0x216,
        I: 0x203,
        r: 0x205,
        d: 0x1f7
      },
      Y = {
        z: 0x207
      },
      l = {
        z: 0x20e
      },
      b = X,
      C = X,
      B = {
        'QcjJR': b(L.z) + 't'
      };
    function O(I) {
      var h = b;
      return z[h(l.z)](this, I) || this;
    }
    return __extends(O, z), Object[C(L.B) + C(L.O)](O[b(L.I) + 'e'], B['QcjJR'], {
      'get': function get() {
        var E = C,
          T = b;
        return this[E(Y.z)][E(0x1f4) + 'nt'];
      },
      'enumerable': !0x1,
      'configurable': !0x0
    }), O[C(0x203) + 'e'][C(L.r) + b(L.d)] = function () {
      return !0x0;
    }, O;
  }(Chat_1['Chat']);
function w(z, B) {
  var O = D();
  return w = function w(I, r) {
    I = I - 0x1ee;
    var d = O[I];
    return d;
  }, w(z, B);
}
exports[F(0x20b) + F(0x1ee)] = MultiPhotoChat;