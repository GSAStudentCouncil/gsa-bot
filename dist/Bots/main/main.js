...
var staffRoom = {}; //BotManager.getChannelById('c1'); FIXME:
var debugRoom = {}; //BotManager.getChannelById('c2'); FIXME:

var studentRooms = {}; // 기수방만 분리
var rooms = {};
var isNumber = function isNumber(name) {
  return /^\d+$/.test(name);
};
for (var _i = 0, _Object$entries = _objectEntries(DB.channels.c2i); _i < _Object$entries.length; _i++) {
  ...
  if (isNumber(name)) studentRooms[name] = BotManager.getChannelById(id);
...
try {
  ...
  bot.on(Event.MESSAGE, function (chat, channel) {
    if (!isNumber(channel.customName)) return;
  ...
  bot.addCommand(new NaturalCommand.Builder().setName('급식', '🍚').setDescription('입력한 시간에 맞춰 급식을 전송합니다. 시간을 생략하면 메시지를 전송한 시각으로 설정됩니다.\n' + '예를 들어, 아침과 점심 시간 사이에 명령어를 호출하면 점심 급식을 알려주고, 점심과 저녁 시간 사이에는 저녁 급식을 알려줍니다.\n' + '또한, 매일 자정 그 날의 모든 급식을 알려주고, 3교시에서 4교시로 가는 쉬는 시간에는 점심, 7교시 이후 청소 시간에 저녁 급식을 정기적으로 전송합니다.').setExamples('그제 급식', '오늘 밥', '모레 급식', '석식', '내일 점심밥', '금요일 아침', '급식 3/29', '급식 다다음주 목요일').setQuery({
    ...
    if (chat.filteredText.replace(/\s+/g, '').length > 3) return;else if (Number.isNaN(datetime)) {
      if (급식 === '조식' || 급식 === '아침') datetime = DateTime.parse('아침');else if (급식 === '중식' || 급식 === '점심') datetime = DateTime.parse('점심');else if (급식 === '석식' || 급식 === '저녁') datetime = DateTime.parse('저녁');else datetime = DateTime.now();
    }
  ...
  bot.addCommand(new StructuredCommand.Builder().setName('공지', '🔔').setDescription("학생회 공지를 전송합니다. 기수를 지정하지 않으면 재학 중인 기수 톡방에 전송됩니다.\n" + "먼저 입력 양식에 맞춰 명령어를 작성해 전송한 뒤, 공지사항을 작성해 한 번 더 전송하세요.\n" + "공지사항 내용 대신 메시지로 '취소'라고 보낼 경우 공지 명령어가 중단됩니다.").setUsage("<\uBD80\uC11C:str> \uC54C\uB9BC <\uAE30\uC218:int[]? min=".concat(DateTime.now().year - 2000 + 15, " max=").concat(DateTime.now().year - 2000 + 17, ">")).setChannels(staffRoom).setExamples(['$user: 생체부 알림', '봇: ' + f.info('$user님, 39, 40, 41기에 생체부로서 공지할 내용을 작성해주세요.'), '$user: 기숙사 3월 기상곡입니다 ...'], ['$user: 정책부 알림 39', '봇: ' + f.info('$user님, 39기에 정책부로서 공지할 내용을 작성해주세요.'), '$user: 정책부에서 야간자율학습 휴대폰 사용 자유 관련 문의를 ...'], ['$user: 홍보부 알림 40 41', '봇: ' + f.info('$user님, 40, 41기에 홍보부로서 공지할 내용을 작성해주세요.'), '$user: 취소', '봇: ' + f.success('취소되었습니다.')]).setExecute(function (self, chat, channel, args) {
    ...
    channel.send(f.info("".concat(chat.user.name, "\uB2D8, ").concat(f.josa(args.부서, '로'), "\uC11C ").concat(args.기수.join(', '), "\uAE30\uC5D0 \uACF5\uC9C0\uD560 \uB0B4\uC6A9\uC744 \uC791\uC131\uD574\uC8FC\uC138\uC694.")));
    ...
    try {
      var _loop = function _loop() {
        var n = _step.value;
        if (!studentRooms[n]) {
          channel.send(f.warn("".concat(n, "\uAE30 \uD1A1\uBC29\uC740 \uC874\uC7AC\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4.")));
          return 1; // continue
        }
        studentRooms[n].send("".concat(self.icon, " ").concat(부서, " \uC54C\uB9BC\n\u2014\u2014\u2014\u2014\u2014\n").concat(chat.text)).then(function () {
          return channel.send(f.success("".concat(n, "\uAE30\uC5D0 ").concat(부서, " \uACF5\uC9C0\uAC00 \uC804\uC1A1\uB418\uC5C8\uC2B5\uB2C8\uB2E4.")));
        })["catch"](function (e) {
          return channel.send(f.warn("".concat(n, "\uAE30\uC5D0 ").concat(부서, " \uACF5\uC9C0 \uC804\uC1A1\uC5D0 \uC2E4\uD328\uD588\uC2B5\uB2C8\uB2E4.\n").concat(e)));
        });
      };
  ...
  bot.addCommand(new StructuredCommand.Builder().setName('도움말', '❓').setDescription("명령어에 대한 상세한 도움말을 표시합니다. 명령어 이름을 생략할 경우, 대신 등록되어 있는 명령어 목록을 전부 출력합니다.").setUsage('도움말 <명령어:str?>').setExamples('도움말', '도움말 공지', '도움말 급식', '도움말 행사').setExecute(function (self, chat, channel, _ref3) {
    ...
    if (명령어 != null) {
      ...
      if (found != null) {
        ...
        return;
      } else channel.send(f.warn("\uBA85\uB839\uC5B4 \uC774\uB984\uC774 '".concat(명령어, "'\uC778 \uBA85\uB839\uC5B4\uB294 \uC874\uC7AC\uD558\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4.")));
    }
    var ret = [];
    ret.push('📦 명령어 목록');
    ret.push('——————');
  ...
  bot.addCommand(new NaturalCommand.Builder().setName('행사', '📅').setDescription('2024년 학사일정을 입력한 날짜 및 기간에 맞춰 알려줍니다.').setExamples('행사 3월 1일', '3월 1일부터 3월 5일까지 학사일정', '다음 주까지 학교 행사').setUseDateParse(true, true).setQuery({
    ...
    if (events.length > 0) channel.send("".concat(self.icon, " ").concat(['이번 주', '오늘'][index], " \uD559\uC0AC\uC77C\uC815\n\u2014\u2014\u2014\u2014\u2014\n").concat(events.join('\n')));else channel.send("".concat(self.icon, " ").concat(['이번 주', '오늘'][index], " \uD559\uC0AC\uC77C\uC815\n\u2014\u2014\u2014\u2014\u2014\n\uD574\uB2F9 \uAE30\uAC04\uC5D0 \uD559\uC0AC\uC77C\uC815\uC774 \uC5C6\uC2B5\uB2C8\uB2E4."));
  }).build());
} catch (err) {
  // debugRoom.send(err)	FIXME:
  // 	.then(() => Log.i('success'))
  // 	.catch(e => Log.e(e.stack ?? e.toString()));
}