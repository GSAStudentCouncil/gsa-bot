...
const {
  ...
  NaturalCommand,
  CommandRegistry
} = require('BotOperator/Command');
const {
  Event
} = require('BotOperator/Event');
const {
  DateTime
} = require('BotOperator/DateTime');
const {
  Channel
} = require('BotOperator/DBManager/classes');
const {
  isNumber,
  isValidChannel,
  compress,
...
let getMeals = (dt, bullet) => {
  let options = [['ATPT_OFCDC_SC_CODE', 'F10'], ['SD_SCHUL_CODE', 7380031], ['MLSV_YMD', dt.toString('YYMMDD')], ['Type', 'xml']];
  try {
    let doc = org.jsoup.Jsoup.connect(`https://open.neis.go.kr/hub/mealServiceDietInfo?${options.map(opt => opt.join('=')).join('&')}`).get();
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      meals[mealType - 1] = String(element.select('DDISH_NM').text()).split(/ (?:\(\d+\.?(?:.\d+)*\))?(?:<br\/>|$)/g).filter(Boolean).map(e => bullet + e).join('\n');
    }
    return meals;
  } catch (e) {
    if (isValidChannel(logRoom)) logRoom.send(`Error:${e.stack}`);
    Log.e(e.stack);
    return [null, null, null];
  }
};
...
let getEvents = (from, to) => {
  ...
  for (let date in events) {
    ...
    if (from.le(dt) && dt.le(to)) {
      ...
      for (let event of events[date].split(/,\s+/)) satisfied[dtString].push(`    · ${event}`);
    }
  }
  let msg = '';
  for (let dtString in satisfied) msg += `${dtString}\n${satisfied[dtString].join('\n')}\n`;
  return msg.slice(0, -1);
};
let 부서명List = ['회장', '부회장', '학생회', '생체부', '환경부', '통계부', '문예부', '체육부', '홍보부', '정책부', '정보부', '총무부'];
let delay = 10 * 1000;

////////////////////// 명령어 선언
try {
  ////////////////////// 디버그 명령어
  bot.addCommand(new StructuredCommand.Builder().setName('디버그', '🔧').setDescription('디버그 모드를 실행하거나 종료합니다. 디버그 모드를 실행하면 테스트방을 제외한 모든 명령어의 사용이 제한됩니다.').setUsage('디버그 <스위치:str?>').setChannels(debugRoom1, debugRoom2).setExamples('디버그 시작', '디버그 종료', '디버그').setExecute((self, chat, channel, {
    스위치
  }) => {
    if (스위치 === '시작') {
  ...
  bot.addCommand(new NaturalCommand.Builder().setName('급식', '🍚').setDescription(`입력한 시간에 맞춰 급식을 전송합니다. 시간을 생략하면 메시지를 전송한 시각으로 설정됩니다.\n예를 들어, 아침과 점심 시간 사이에 명령어를 호출하면 점심 급식을 알려주고, 점심과 저녁 시간 사이에는 저녁 급식을 알려줍니다.\n또한, 매일 자정 그 날의 모든 급식을 알려주고, 3교시에서 4교시로 가는 쉬는 시간에는 점심, 7교시 이후 청소 시간에 저녁 급식을 정기적으로 전송합니다.`).setExamples('그제 급식', '오늘 밥', '모레 급식', '석식', '내일 점심밥', '금요일 아침', '급식 3/29', '급식 다다음주 목요일').setQuery({
  ...
  }).setUseDateParse(0, false, false).setExecute((self, chat, channel, {
    급식,
    datetime
  }) => {
    // if (isNaN(datetime)) {
    ...
    if (index === 0) {
      ...
      if (meals.every(e => e == null)) return;
    ...
    // 첫 번째 크론이 아니면 해당 시간의 급식만 출력
      ...
      let meals = getMeals(dt, '· ');
      if (index === 1 && meals[1] != null) msg = `🍔 ${dt.humanize(true)} 중식\n——\n${meals[1]}`;else if (index === 2 && meals[2] != null) msg = `🍱 ${dt.humanize(true)} 석식\n——\n${meals[2]}`;
    }
  ...
  bot.addCommand(new StructuredCommand.Builder().setName('공지', '📢').setDescription(`학생회 공지를 전송합니다. 기수를 지정하지 않으면 재학 중인 기수 톡방에 전송됩니다.\n먼저 입력 양식에 맞춰 명령어를 작성해 전송한 뒤, 공지사항(메시지, 사진, 영상, 파일)을 작성해 한 번 더 전송하세요.\n공지사항 내용 대신 메시지로 \'취소\'라고 보낼 경우 공지 명령어가 중단됩니다.\n<부서>에는 다음과 같은 문자열이 들어갑니다. ${부서명List.join(', ')}`).setUsage(`<부서:str> 알림 <기수:int[]? min=${DateTime.now().year - 2000 + 15} max=${DateTime.now().year - 2000 + 17}>`).setChannels(staffRoom).setExamples(['$user: 생체부 알림', '봇: $user님, 39, 40, 41기에 생체부로서 공지할 내용을 작성해주세요.', '$user: 기숙사 3월 기상곡입니다 ...'], ['$user: 정책부 알림 39', '봇: $user님, 39기에 정책부로서 공지할 내용을 작성해주세요.', '$user: 정책부에서 야간자율학습 휴대폰 사용 자유 관련 문의를 ...'], ['$user: 홍보부 알림 40 41', '봇: $user님, 40, 41기에 홍보부로서 공지할 내용을 작성해주세요.', '$user: 취소', '봇: 취소되었습니다.']).setExecute((self, chat, channel, {
    ...
    let msg;
    if (chat.isFile()) msg = `📄 ${shortURL(chat.file.url)} (${chat.file.name}, ${prettyBytes(chat.file.size)})`;else if (chat.isPhoto()) msg = `🖼 ${shortURL(chat.photo.url)} (${prettyBytes(chat.photo.s)})`;else if (chat.isMultiPhoto()) msg = chat.photoList.imageUrls.map((url, i) => `🖼 ${shortURL(url)} (${prettyBytes(chat.photoList.sl[i])})`).join('\n');else if (chat.isVideo()) msg = `🎥 ${shortURL(chat.video.url)} (${prettyDuration(chat.video.d)}, ${prettyBytes(chat.video.s)})`;else msg = chat.text;
    if (bot.isDebugMod) {
      if (channel.id === debugRoom1.id) debugRoom2.send(`${self.icon} ${부서} 알림\n——\n${msg}`).then(() => channel.success(`debugRoom2에 ${부서} 공지가 전송되었습니다.`)).catch(e => channel.warn(`debugRoom2에 ${부서} 공지 전송에 실패했습니다.\n${e}`));else if (channel.id === debugRoom2.id) debugRoom1.send(`${self.icon} ${부서} 알림\n——\n${msg}`).then(() => channel.success(`debugRoom1에 ${부서} 공지가 전송되었습니다.`)).catch(e => channel.warn(`debugRoom1에 ${부서} 공지 전송에 실패했습니다.\n${e}`));
    } else {
      // 공지 전송
      for (let n of 기수) {
        if (!studentRooms[n]) {
        ...
        studentRooms[n].send(`${self.icon} ${부서} 알림\n——\n${msg}`).then(() => channel.success(`${n}기에 ${부서} 공지가 전송되었습니다.`)).catch(e => channel.warn(`${n}기에 ${부서} 공지 전송에 실패했습니다.\n${e}`));
      }
    }
  }).build());

  ////////////////////// 도움말 명령어
  bot.addCommand(new StructuredCommand.Builder().setName('도움말', '❓').setDescription('명령어에 대한 상세한 도움말을 표시합니다. 명령어 이름(또는 아이콘)을 생략할 경우, 대신 등록되어 있는 명령어 목록을 전부 출력합니다.').setUsage('도움말 <명령어:str?>').setExamples('도움말', '도움말 공지', '도움말 급식', '도움말 행사', '도움말 📅', '도움말 🍚').setExecute((self, chat, channel, {
    명령어
  }) => {
    // 명령어 이름이 주어진 경우
    if (명령어 != null) {
      ...
      if (found != null) channel.send(found.manual({
        user: chat.user.name
      }));else channel.warn(`명령어 이름이 '${명령어}'인 명령어는 존재하지 않습니다.`);
      return;
    }
    let ret = [];
    ret.push('📦 명령어 목록');
  ...
  bot.addCommand(new NaturalCommand.Builder().setName('일정', '📅').setDescription('2024년 학사일정을 입력한 날짜 및 기간에 맞춰 알려줍니다.').setExamples('행사 3월 1일', '3월 1일부터 3월 5일까지 학사일정', '다음 주까지 학교 행사').setUseDateParse(0, true).setQuery({
    ...
    let msg;
    if (eventStr.length > 0) msg = `${self.icon} 학사일정 (${from.humanize(true)} ~ ${to.humanize(true)})\n——\n${eventStr}`;else msg = `${self.icon} 학사일정 (${from.humanize(true)} ~ ${to.humanize(true)})\n——\n해당 기간에 학사일정이 없습니다.`;
    if (bot.isDebugMod) debugRoom1.send(msg);else channel.send(msg);
  }).setCronJob([{
    cron: '0 0 * * 1',
    ...
    if (bot.isDebugMod) debugRoom1.send(`${self.icon} ${['이번 주', '오늘'][index]} 학사일정\n——\n${eventStr}`);else {
      for (let 기수 in studentRooms) {
