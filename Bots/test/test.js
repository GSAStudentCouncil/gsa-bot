...
var { DateTime } = require('BotOperator/DateTime');
...
var getMeals = (dt, bullet) => {
    ...
    try {
        var doc = Jsoup.connect(
            "https://open.neis.go.kr/hub/mealServiceDietInfo?" + options.map(opt => opt.join('=')).join('&')).get();
        ...
        for (let i = 0; i < elements.length; i++) {
            ...
            var dishName = element.select('DDISH_NM').
                text().
                split(/ (?:\(\d+\.?(?:.\d+)*\))?(?:<br\/>|$)/g).
                filter(Boolean).
                map(e => bullet + e).
