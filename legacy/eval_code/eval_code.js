...
function rstr2hex(input){try{hexcase}catch(e){hexcase=0;}var hex_tab=hexcase?"0123456789ABCDEF":"0123456789abcdef";var output="",x;for(var i=0;i<input.length;i++){x=input.charCodeAt(i);output+=hex_tab.charAt((x>>>4)&0x0F)+hex_tab.charAt(x&0x0F);}return output;}
...
function requestSSL(url) {
    ...
    sslContext.init(null, [
        new JavaAdapter(javax.net.ssl.X509TrustManager, {
    ...
    if (params.msg.startsWith("*ev")) {
        if (admin.includes(profileCode).valueOf()) {
            ...
            try {
                ...
                endTime = Date.now();
                operatedTime = endTime - startTime;
                params.replier.reply("runtime: " + operatedTime + "ms");
            }
            catch (e) {
                params.replier.reply("ERROR\nName: " + e.name + "\nMessage: " + e.message + "\nLine: " + e.lineNumber + "\n" + e.stack);
            }
        }
    }
}
