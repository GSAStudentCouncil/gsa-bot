...
class datetime {
	...
	static parse(dateString, locale = 'ko-KR') {  // TODO: implement
		...
		const parse3 = () => {
			...
			let tokens = dateString.split(' ').map(_get).flat();
			...
			const absoluteParse = () => {
				...
				for (let i = 0; i < tokens.length; i++) {
					...
					if (keywords.units.has(token)) {    // year, month, week, day, hour, minute, second, millisecond 감지
						...
						if (dateObject['hour'] < 12) {  // 7시가 오후 7시인지, 오전 7시인지 가림
							if (keywords.meridiems.has(token)) {
								dateObject['hour'] += (token === 'pm' ? 12 : 0);
							}
							else {    // meridiem이 없을 경우 기본값 pm 으로 설정 (7시 = 오후 7시)
								dateObject['hour'] += 12;
							}
						}
					}
			...
			const relativeParse = () => {
				...
				for (let i = 0; i < tokens.length; i++) {
					...
					if (keywords.relative.has(token)) {     // ago, after
						// TODO: '3일 전' -> [3, 'day', 'ago'] -> ['#-3', 'day']
						if (token === 'ago')
							tokens = tokens.map(t => typeof t === 'number' ? -t : t);
						// token === 'after' 는 아무 영향 안 줌
			...
			let absolute = absoluteParse();
			let relative = relativeParse();
			
			console.log(absolute, relative);    // FIXME: debug
			
			if (Object.keys(absolute).length > 0)
				return new datetime(absolute);
			else if (Object.keys(relative).length > 0)
				return new datetime(relative);
		};
